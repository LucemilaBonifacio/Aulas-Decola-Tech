...
public interface InscricaoRepository extends JpaRepository<Inscricao, Integer>{
	...
	@Query("SELECT new com.avanade.decolatech.rh.dto.CandidatoInscricoesDTO(cd.cpf,"
			+ "cd.nome, ic.dataInscricao) FROM Candidato cd "
			+ "INNER JOIN cd.inscricoes ic ON cd.cpf = ic.candidato.cpf")
	List<CandidatoInscricoesDTO> listarInscricoesPorCandidatoDTO();
	
	
	@Query("SELECT new com.avanade.decolatech.rh.dto.CandidatoInscricoesDTO(cd.cpf,"
			+ "cd.nome, ic.dataInscricao) FROM Candidato cd "
			+ "INNER JOIN cd.inscricoes ic ON cd.cpf = ic.candidato.cpf WHERE cd.cpf = :valorcpf")
